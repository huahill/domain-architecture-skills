...
Use a transaction boundary only for an application workflow that requires atomic changes. The preferred approach is for application services to inject jFoundry `TransactionRunner` from `jfoundry-transaction-core` (Spring Boot auto-configures `SpringTransactionRunner`) and call `run` / `call` directly inside the service method. Do not create a project-local `UnitOfWork` port or a `Transactions` helper. Java/Spring code talks about transactions, not Unit of Work. Keep aggregate load and tracked modification in the same transaction. Do not create a separate `Transactional*UseCase` decorator for every use case; inline the transaction boundary in the service that owns the workflow. Workers must call `TransactionRunner` for the short database section and keep outbound HTTP outside it. `run` / `call` do not declare checked exceptions; translate adapter-level checked failures before they enter the callback.
