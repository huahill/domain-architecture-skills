...
- Application and worker/boot composition classes that call `AggregateRepository.findById` or `findByIdForUpdate` must own a transaction boundary: a `UnitOfWork` / equivalent, or a `Transactional*` use-case decorator that uses `TransactionTemplate`. Record a red architecture test that rejects an aggregate load without that boundary.
