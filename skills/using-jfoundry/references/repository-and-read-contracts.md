...
- Command-side `AggregateRepository.findById` is a tracked load, not a generic query. Call it only inside `TransactionRunner` or a transactional use-case decorator. External SDK or HTTP preparation that does not mutate the aggregate belongs on a lookup contract.
